...
{if isset($orderby) AND isset($orderway)}
	{if !isset($request)}
		...
		{if isset($smarty.get.id_category) && $smarty.get.id_category}
			{assign var='request' value=$link->getPaginationLink('category', $category, false, true)}	{elseif isset($smarty.get.id_manufacturer) && $smarty.get.id_manufacturer}
	...
	<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
		<div class="select selector1">
			<label class="visuallyhidden" for="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}">{l s='Sort by'}</label>
	...
	<!-- /Sort products -->
	<ul class="grid-list-buttons display">
		<li id="grid">
			<a rel="nofollow" href="#" title="{l s='Grid'}">
				<i class="flaticon-grid"></i>
				<span class="visuallyhidden">{l s='Grid'}</span>
			</a>
		</li>
		<li id="list">
			<a rel="nofollow" href="#" title="{l s='List'}">
				<i class="flaticon-menu"></i>
				<span class=visuallyhidden"{l s='List'}</span>
			</a>
		</li>
	</ul><!-- /.grid-list-buttons -->
